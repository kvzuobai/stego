...
import {ITicketTransistors, IProtocolFee} from "./NerveSeason.sol";

/// @title NerveSeason2
/// @notice Season 2+ of Nerve. Same game as NerveSeason (Season 1, left unchanged as deployed), plus:
///   * Diamond hands: players who never exit get their pot weight multiplied by (1 + diamondBonusBps).
///     Leaving even once, even partially, loses the bonus for the rest of the season.
///   * On-chain player list and a paged `standings` view, so the site can show a live leaderboard
///     without reading event logs.
///
/// A bank-run game whose exit rule is a TapeOut circuit.
///
...
contract NerveSeason2 is ReentrancyGuard, ERC1155Holder {
    ...
    uint256 public immutable diamondBonusBps; // extra pot weight for players who never exit
    ...
    uint256 public finalPoints; // total pot weight at settlement (diamond bonus included)

    mapping(address => bool) public exited; // ever left (fully or partly) this season
    uint256 public diamondPoints; // sum of points of players who never exited
    ...
    constructor(Params memory p) {
        ...
        if (
            nIn != 16 || nOut != 2 || nState != 0 || p.joinDeadline > p.seasonEnd || p.seasonEnd <= block.timestamp
                || p.baseFeeBps > p.throttleFeeBps || p.throttleFeeBps > 2_000 || p.designerCutBps > 2_000 || p.diamondBonusBps > 5_000
        ...
        diamondBonusBps = p.diamondBonusBps;
    }
    ...
    /// @notice A player's pot weight: points, plus the diamond bonus if they never exited.
    function weightOf(address player) public view returns (uint256) {
        uint256 pts = points[player];
        return exited[player] ? pts : pts + pts * diamondBonusBps / BPS;
    }

    /// @notice Sum of all players' weights.
    function totalWeight() public view returns (uint256) {
        return totalPoints + diamondPoints * diamondBonusBps / BPS;
    }
    ...
    function projectedPayout(address player) external view returns (uint256 principal_, uint256 share) {
        ...
        if (settled) return (principal_, finalPoints == 0 ? 0 : finalPot * weightOf(player) / finalPoints);
        uint256 tw = totalWeight();
        if (tw == 0) return (principal_, 0);
        share = _potAfterCut() * weightOf(player) / tw;
    }
    ...
    /// @notice Paged leaderboard data: address, deposit, pot weight and diamond status per player.
    function standings(uint256 offset, uint256 limit)
        external
        view
        returns (address[] memory who, uint256[] memory deposit, uint256[] memory weight, bool[] memory diamond)
    {
        uint256 n = playerList.length;
        uint256 end = offset + limit > n ? n : offset + limit;
        uint256 len = end > offset ? end - offset : 0;
        who = new address[](len);
        deposit = new uint256[](len);
        weight = new uint256[](len);
        diamond = new bool[](len);
        for (uint256 i = 0; i < len; i++) {
            address a = playerList[offset + i];
            who[i] = a;
            deposit[i] = principal[a];
            weight[i] = weightOf(a);
            diamond[i] = !exited[a] && principal[a] > 0;
        }
    }
    ...
    function joinQuote(address player, uint256 amount) external view returns (uint256 share, uint256 ticket) {
        ...
        uint256 w = exited[player] ? pts : pts + pts * diamondBonusBps / BPS;
        uint256 tw = totalWeight();
        uint256 mine = weightOf(player);
        share = _potAfterCut() * (mine + w) / (tw + w) - (tw == 0 ? 0 : _potAfterCut() * mine / tw);
    }
    ...
    function boostQuote(address player, uint256 units) external view returns (uint256 extraShare, uint256 cost) {
        ...
        uint256 w = exited[player] ? extra : extra + extra * diamondBonusBps / BPS;
        uint256 pc = _potAfterCut();
        uint256 tw = totalWeight();
        uint256 mine = weightOf(player);
        extraShare = pc * (mine + w) / (tw + w) - pc * mine / tw;
    }
    ...
    function join() external payable nonReentrant {
        ...
        totalPoints += pts;
        if (!exited[msg.sender]) diamondPoints += pts;
    ...
    function boost(uint256 units) external payable nonReentrant {
        ...
        totalPoints += extra;
        if (!exited[msg.sender]) diamondPoints += extra;
    ...
    function exit(uint256 amount) external nonReentrant {
        ...
        totalPrincipal -= amount;
        if (!exited[msg.sender]) {
            exited[msg.sender] = true; // leaving at all forfeits the diamond bonus
            diamondPoints -= points[msg.sender];
        }
    ...
    function claim() external nonReentrant {
        ...
        uint256 share = finalPot * weightOf(msg.sender) / finalPoints;
